...
<section class="list-wrap">
    ...
    <Modal
      v-model="showeUpdateModel"
      title="提示"
    >
      <Form ref="updateForm" :model="updateForm" :rules="updateFormRule" inline class="model-form">
        <FormItem prop="id" label="id">
          <Input type="text" v-model="updateForm.id" placeholder="不用填写" readonly>
          </Input>
        </FormItem>
        <FormItem prop="username" label="username">
          <Input type="text" v-model="updateForm.username" placeholder="Username">
            <Icon type="ios-person-outline" slot="prepend"></Icon>
          </Input>
        </FormItem>
        <FormItem prop="realname" label="真实姓名">
          <Input v-model="updateForm.realname" placeholder="realname">
            <Icon type="ios-lock-outline" slot="prepend"></Icon>
          </Input>
        </FormItem>
        <FormItem prop="email" label="email">
          <Input v-model="updateForm.email" placeholder="email">
            <Icon type="ios-lock-outline" slot="prepend"></Icon>
          </Input>
        </FormItem>
        <FormItem prop="company" label="company">
          <Input  v-model="updateForm.password"  placeholder="company">
            <Icon type="ios-lock-outline" slot="prepend"></Icon>
          </Input>
        </FormItem>
        <FormItem prop="score" label="score">
          <Input  v-model="updateForm.score"  placeholder="score">
            <Icon type="ios-lock-outline" slot="prepend"></Icon>
          </Input>
        </FormItem>
        <FormItem prop="telephone" label="telephone">
          <Input  v-model="updateForm.telephone"  placeholder="telephone">
            <Icon type="ios-lock-outline" slot="prepend"></Icon>
          </Input>
        </FormItem>
        <FormItem prop="cancer" label="cancer">
          <Input v-model="updateForm.cancer"  placeholder="cancer">
            <Icon type="ios-lock-outline" slot="prepend"></Icon>
          </Input>
        </FormItem>
      </Form>
      <div slot="footer">
        <Button size="large" long  @click="check('updateForm',1)">修改</Button>
      </div>
    </Modal>
    <Modal
    ...
    data() {
      return {
        ...
        updateFormRule: {
          username: [
            { required: true, message: 'Please fill in the user name', trigger: 'blur' },
            { type: 'string', min: 6, message: 'The length cannot be less than 6 bits', trigger: 'blur' }
          ],
          realname: [
            { required: true, message: 'Please fill in the realname', trigger: 'blur' }
          ],
